				...
				if (!isNull _instigator) then {
					if (isPlayer _instigator) then {
						...
						if ((random 1) > 0.5) then {
							['sideChat',[EAST,'HQ'],(format ['%1 (%2) destroyed the radio tower! Good work soldiers!',_name,(groupID (group _instigator))])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
						} else {
							['sideChat',[EAST,'HQ'],(format ['%1 (%2) brought down the tower! Great work boys!',_name,(groupID (group _instigator))])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
						};
