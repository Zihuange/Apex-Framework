				...
				if (!isNull _instigator) then {
					if (isPlayer _instigator) then {
						...
						if ((random 1) > 0.5) then {
							['sideChat',[EAST,'HQ'],(format ['%1 (%2) %3',_name,(groupID (group _instigator)),localize 'STR_QS_Chat_008'])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
						} else {
							['sideChat',[EAST,'HQ'],(format ['%1 (%2) %3',_name,(groupID (group _instigator)),localize 'STR_QS_Chat_009'])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
						};
