...
if (fileExists (format ['@Apex_cfg_opfor\%1\roles.sqf',worldName])) then {
	diag_log '***** DEBUG ***** COMPILING TERRAIN SPECIFIC ROLES DATA';
	call (compileScript [(format ['@Apex_cfg_opfor\%1\roles.sqf',worldName])]);
} else {
...
if (fileExists (format ['@Apex_cfg_opfor\%1\arsenal.sqf',worldName])) then {
	diag_log '***** DEBUG ***** COMPILING TERRAIN SPECIFIC ARSENAL DATA';
	missionNamespace setVariable ['QS_data_arsenal',(compileScript [(format ['@Apex_cfg_opfor\%1\arsenal.sqf',worldName]),TRUE]),TRUE];
} else {
