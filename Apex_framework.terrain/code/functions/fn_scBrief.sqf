	...
	if (_scWinningSide in [0,2]) then {
		...
		[
			[],
			{
				...
				if (!isStreamFriendlyUIEnabled) then {
					_text = parseText format ["<t align='center' size='2'>%2</t><br/><br/><img size='7' image='%1'/><br/><br/>%3",(missionNamespace getVariable ['QS_missionConfig_textures_defaultFlag','a3\data_f\flags\flag_csat_co.paa']),localize 'STR_QS_Hints_129',localize 'STR_QS_Hints_130'];
					(missionNamespace getVariable 'QS_managed_hints') pushBack [2,FALSE,7.5,-1,_text,[],-1];
				};
			}
		] remoteExec ['spawn',-2,FALSE];
