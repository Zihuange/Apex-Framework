...
if (_type isEqualTo 0) exitWith {
	...
	if ((missionNamespace getVariable ['QS_missionConfig_arty',0]) isNotEqualTo 0) then {
		...
		// M4 Scorcher
		if (_artyType isKindOf 'O_MBT_02_arty_F') then {
		...
		// M5 MLRS
		if ((_artyType isKindOf 'I_TRUCK_02_mrl_F') || (_artyType isKindOf 'I_E_TRUCK_02_mrl_F')) then {
...
if (_type isEqualTo 1) exitWith {
	if ((missionNamespace getVariable ['QS_missionConfig_arty',0]) isNotEqualTo 0) then {
		...
		if (alive _arty) then {
			...
			['sideChat',[EAST,'HQ'],(format ['%1 %2',_dn,localize 'STR_QS_Chat_024'])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
			// M4 Scorcher
			if (_arty isKindOf 'O_MBT_02_arty_F') then {
				['setVehicleAmmo',_arty,1] remoteExec ['QS_fnc_remoteExecCmd',_arty,FALSE];
			};
			// M5 MLRS
			if (_arty isKindOf 'I_TRUCK_02_mrl_F') then {
				[
					[_arty,_ammo_MLRS],
					{      
