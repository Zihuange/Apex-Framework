...
private _reinforceGroup = createGroup [WEST,TRUE];
...
if (
	...
	{(([4,WEST,(missionNamespace getVariable 'QS_aoPos'),1500] call (missionNamespace getVariable 'QS_fnc_AIGetKnownEnemies')) <= 1)} &&
	{(([3,WEST,(missionNamespace getVariable 'QS_aoPos'),2000] call (missionNamespace getVariable 'QS_fnc_AIGetKnownEnemies')) <= 1)}
) then {
