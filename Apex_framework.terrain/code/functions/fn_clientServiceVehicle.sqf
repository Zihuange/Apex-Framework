		...
		if (player getUnitTrait 'QS_trait_fighterPilot') then {
			if ((toLowerANSI (typeOf _v)) in [
				...
				if (diag_tickTime > (uiNamespace getVariable ['QS_fighterPilot_lastMsg',(diag_tickTime - 1)])) then {
					...
					['sideChat',[EAST,'AirBase'],(format ['%3 %2 (%1)',(getText (configFile >> 'CfgVehicles' >> (typeOf _v) >> 'displayName')),profileName,localize 'STR_QS_Chat_029'])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
				};
			};
		};
	};
