...
_jetPilot = (missionNamespace getVariable 'QS_enemyCasGroup') createUnit [QS_core_units_map getOrDefault [toLowerANSI 'b_fighter_pilot_f','o_fighter_pilot_f'],[-100,-100,0],[],0,'NONE'];
...
if (!((toLowerANSI (typeOf _jetActual)) in ['c_plane_civil_01_racing_f','c_plane_civil_01_racing_f'])) then {
	if ((toLowerANSI (typeOf _jetActual)) in ['b_plane_fighter_01_f','b_plane_fighter_01_stealth_f','o_plane_fighter_02_cluster_f']) then {
...
if (!((toLowerANSI _jetSelect) in ['b_plane_fighter_01_stealth_f'])) then {
