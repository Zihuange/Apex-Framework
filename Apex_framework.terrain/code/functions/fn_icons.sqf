...
_QS_fnc_iconUnits = {
	...
	private _au = [];
	_isAdmin = ((_QS_ST_X # 86) && {((call (missionNamespace getVariable 'BIS_fnc_admin')) isEqualTo 2)});
	if ((QS_player getVariable ['QS_unit_side',EAST]) isNotEqualTo CIVILIAN) then {
		if (!(_QS_ST_X # 74)) then {
	...
	if ((_QS_ST_X # 62)) then {
		_as pushBack (QS_player getVariable ['QS_unit_side',EAST]);
	} else {
		if (isMultiplayer) then {
			if (_isAdmin) then {
				...
				_as pushBack (QS_player getVariable ['QS_unit_side',EAST]);
				{
					if (((QS_player getVariable ['QS_unit_side',EAST]) getFriend _x) > 0.6) then {
...
_QS_fnc_onMapSingleClick = {
	...
	if ((!(_alt)) && (!(_shift))) then {
		...
		if (
			...
			{((side (effectiveCommander _vehicle)) isEqualTo (QS_player getVariable ['QS_unit_side',EAST]))} &&
...
_QS_fnc_onGroupIconClick = {
	...
	if ((side _group) isNotEqualTo (QS_player getVariable ['QS_unit_side',EAST])) exitWith {
...
_QS_fnc_onGroupIconOverEnter = {
	if ((side (_this # 1)) isNotEqualTo (QS_player getVariable ['QS_unit_side',EAST])) exitWith {};
};
