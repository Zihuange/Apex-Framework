...
if (_type isEqualTo 0) exitWith {
	...
	if (missionNamespace getVariable ['QS_client_showKnownEnemies',TRUE]) then {
		...
		if ((lifeState _player) in ['HEALTHY','INJURED']) then {
			...
			_remoteTargets = [(listRemoteTargets (_player getVariable ['QS_unit_side',EAST])),0] call (missionNamespace getVariable 'QS_fnc_listRemoteTargets');
...
if (_type isEqualTo 1) exitWith {
	...
	_allPlayers = (units EAST) call (missionNamespace getVariable 'QS_fnc_arrayShuffle');
