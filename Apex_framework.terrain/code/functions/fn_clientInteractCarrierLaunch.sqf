...
if (!(_cameraOn getVariable ['QS_carrier_launch',FALSE])) then {
	...
	if (!(_launchPos isEqualTo [])) then {
		...
		_launchCancel = {
			...
			if (!isEngineOn _cameraOn) exitWith {
				50 cutText ['必须启动引擎才能弹射','PLAIN',0.5];
			};
			if (_cameraOn getVariable ['QS_carrier_launch',FALSE]) then {
				...
				50 cutText ['弹射已取消','PLAIN DOWN',0.75];
			};
			_cameraOn removeAction _actionID;
			missionNamespace setVariable ['QS_client_action_carrierLaunchCancel',[],FALSE];
		};
		_launchCancelAction = _cameraOn addAction ['取消弹射',_launchCancel,[_cameraOn],0,FALSE,TRUE,'','TRUE',-1,FALSE,''];
	...
	_cameraOn spawn {
		...
		if (diag_tickTime > (uiNamespace getVariable ['QS_fighterPilot_lastMsg',(diag_tickTime - 1)])) then {
			...
			[63,[4,['CAS_1',['','我方固定翼支援升空！']]]] remoteExec ['QS_fnc_remoteExec',-2,FALSE];
			['sideChat',[WEST,'AirBase'],(format ['我方空中单位 (%1) 起飞，正在准备执行CAS任务。 飞行员： %2',(getText (configFile >> 'CfgVehicles' >> (typeOf _this) >> 'displayName')),profileName])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
		};
