			...
			private _scorePlayerCoef = 1 min (_playerCount / 50);
			{
				...
				if (_isActive) then {
					if (_tickTime > _nextEvaluationTime) then {
						...
						_unitsConvertingCoef = (0 max _unitsConvertingInArea min 10) / ([10,10] select ((_sidesCanConvert select 0) isEqualTo _enemySide));
						...
						if (_conversionValue isEqualTo 0) then {
							...
							_sideIndex = _sides find (_sidesOwnedBy select 0);
							if (!((flagTexture (_flagData select 0)) == (_sidesFlagsTextures select _sideIndex))) then {
								(_flagData select 0) setFlagTexture (_sidesFlagsTextures select _sideIndex);
							};
							{
								if (!((side _x) isEqualTo (_sidesOwnedBy select 0))) then {
									_x setSide (_sidesOwnedBy select 0);
								};
							} forEach _locationData;
							{
								if (!((toLower (markerColor _x)) isEqualTo (_markerColors select _sideIndex))) then {
							...
							if ((_sidesOwnedBy select 0) isEqualTo WEST) then {
								[
									...
									[
										(_taskData select 1),
										(format ['Lost %1!',(_taskData select 2)])
									]
								] remoteExec ['QS_fnc_showNotification',-2,FALSE];
								{
									if (!isNull _x) then {
										if (!((getPosWorld _x) isEqualTo _centerPos)) then {
								...
								{
									if (!isNull _x) then {
										if (!((getPosWorld _x) isEqualTo [-1000,-1000,0])) then {
											_x setPosWorld [-1000,-1000,0];
											_x enableSimulationGlobal TRUE;
										};
									};							
								} forEach (_sectorAreaObjects select 1);							
							} else {
								[
									'SC_UPDATE_GOOD',
									[
										(_taskData select 1),
										(format ['Captured %1!',(_taskData select 2)])
									]
								] remoteExec ['QS_fnc_showNotification',-2,FALSE];
								{
									if (!((getPosWorld _x) isEqualTo [-1000,-1000,0])) then {
							...
							((_sectorAreaObjects select 0) + (_sectorAreaObjects select 1)) spawn {
								uiSleep 1;
								{
						...
						/*/if ((count _playersAO) > 0) then {/*/
							if (_conversionValue >= _conversionValuePrior) then {
								if ((_scoreSides select _sideIndex) <= _scoreWin) then {
									...
									_scoreSides set [_sideIndex,((_scoreSides select _sideIndex) + _scoreCoef)];
								};
							};
						/*/};/*/
					};
...
if (_type isEqualTo 'EXIT') exitWith {
	...
	{
		['REMOVE',(_x select 0)] call (missionNamespace getVariable 'QS_fnc_sc');
	} forEach (missionNamespace getVariable 'QS_virtualSectors_data');
	missionNamespace setVariable ['QS_virtualSectors_enabled',FALSE,TRUE];
...
if (_type isEqualTo 'REMOVE') exitWith {
	...
	if (!(_index isEqualTo -1)) then {
		...
		deleteVehicle (_flagData select 0);
		private _array = [];
		{
			_array = _x;
			{
...
if (_type isEqualTo 'ACT') exitWith {
	...
	if (!(_index isEqualTo -1)) then {
		...
		if (!(_isActive isEqualTo _activationState)) then {
			diag_log '***** SC ACT * 2 *****';
