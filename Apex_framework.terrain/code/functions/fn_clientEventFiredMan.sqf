	...
	if (
		...
		if ((toLowerANSI _magazine) in QS_core_classNames_grenades) then {
			0 = [_projectile] spawn {
				params ['_projectile'];
				uiSleep 3.666;
				private _playersNearby = ([(getPosATL _projectile),15,[EAST],allPlayers,0] call (missionNamespace getVariable 'QS_fnc_serverDetector')) select {(_x isNotEqualTo player)};
		...
		if (_nearEntities isNotEqualTo []) then {
			{
				if (
					...
					{((side _x) in [EAST,CIVILIAN,sideFriendly])}
				) exitWith {
					deleteVehicle _projectile;
				};
				...
				if (_cursorIntersections isNotEqualTo []) then {
					...
					if (
						(
							...
							(
								((side _objectParent) in [WEST,RESISTANCE]) || 
								{(_canAttachExp)} ||
