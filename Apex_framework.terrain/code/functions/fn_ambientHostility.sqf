...
if (_type isEqualTo 0) exitWith {
	...
	for '_x' from 0 to 49 step 1 do {
		...
		if (
			...
			{(([(AGLToASL _spawnPosition),_checkVisibleDistance,_playersOnGround,[EAST,CIVILIAN,SIDEFRIENDLY],0,0] call (missionNamespace getVariable 'QS_fnc_isPosVisible')) <= 0.1)}
	...
	if (_isVehicle) then {
		...
		for '_x' from 0 to 1 step 1 do {
			...
			for '_x' from 0 to (_grpSize - 1) step 1 do {
				...
				if ((toLowerANSI _unitType) in ['b_g_soldier_f','b_g_soldier_lite_f','i_c_soldier_bandit_6_f','i_c_soldier_para_1_f']) then {
