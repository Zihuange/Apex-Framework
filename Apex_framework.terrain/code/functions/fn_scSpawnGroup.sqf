...
if (_sectorData isEqualType []) exitWith {
			...
			if ((_players inAreaArray [_position,250,250,0,FALSE]) isEqualTo []) then {
				if ((((_position select [0,2]) nearRoads 25) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))}) isEqualTo []) then {
					if (!([_position,_centerPos,25] call (missionNamespace getVariable 'QS_fnc_waterIntersect'))) then {
						if (([(AGLToASL _position),_checkVisibleDistance,_playersOnGround,[EAST,CIVILIAN,SIDEFRIENDLY],0,0] call (missionNamespace getVariable 'QS_fnc_isPosVisible')) <= 0.1) then {
							_positionFound = TRUE;
						};
					};
				};
			...
			if (({(alive _x)} count (missionNamespace getVariable ['QS_AI_insertHeli_helis',[]])) < (missionNamespace getVariable ['QS_AI_insertHeli_maxHelis',3])) then {
				if (diag_tickTime > ((missionNamespace getVariable ['QS_AI_insertHeli_lastEvent',-1]) + (missionNamespace getVariable ['QS_AI_insertHeli_cooldown',900]))) then {
					if ((missionNamespace getVariable ['QS_AI_insertHeli_spawnedAO',0]) < (missionNamespace getVariable ['QS_AI_insertHeli_maxAO',3])) then {
						if (([4,WEST,(missionNamespace getVariable 'QS_aoPos'),2000] call (missionNamespace getVariable 'QS_fnc_AIGetKnownEnemies')) < 2) then {
							if (([3,WEST,(missionNamespace getVariable 'QS_aoPos'),2000] call (missionNamespace getVariable 'QS_fnc_AIGetKnownEnemies')) < 2) then {
								...
								_heliInsert = TRUE;
								_infantryGroupType = selectRandomWeighted ['BUS_InfSquad',0.5,'BUS_InfAssault',0.5];
								_grp = [_position,(random 360),WEST,_infantryGroupType,FALSE,grpNull,TRUE,TRUE] call (missionNamespace getVariable 'QS_fnc_spawnGroup');
								[
									_centerPos,
									WEST,
									(['B_Heli_Transport_03_F','B_Heli_Transport_03_black_F'] select (worldName isEqualTo 'Tanoa')),
									0.75,
									[],
									(((random 1) > 0.85) && ((count allPlayers) > 10)),
									(['B_Heli_Attack_01_dynamicLoadout_F','B_Heli_Attack_01_dynamicLoadout_F'] select (worldName isEqualTo 'Tanoa')),
									(units _grp),
									FALSE
	...
	if (!(_heliInsert)) then {
		if (missionNamespace getVariable ['QS_virtualSectors_sub_3_active',TRUE]) then {
			_infantryGroupType = selectRandomWeighted [
				...
				'BUS_InfTeam_AT',2,
				'BUS_InfTeam_HAT',2,
				(['IRG_InfTeam','BUS_T_CTRGPatrol'] select ((count _players) > 30)),1,
				'BUS_reconSentry',1
			];
		} else {
			_infantryGroupType = selectRandomWeighted [
	...
	[(units _grp),([1,1] select (_infantryGroupType in ['BUS_reconPatrol','BUS_T_CTRGPatrol','BUS_reconSentry']))] call (missionNamespace getVariable 'QS_fnc_serverSetAISkill');
...
if (_sectorData isEqualType 0) exitWith {
	if (_sectorData isEqualTo -1) then {
		...
		for '_x' from 0 to 1 step 0 do {
			...
			if ((_position distance2D _centerPos) < 600) then {
				if ((_players inAreaArray [_position,250,250,0,FALSE]) isEqualTo []) then {
					if (!([_position,_centerPos,25] call (missionNamespace getVariable 'QS_fnc_waterIntersect'))) then {
						if (([(AGLToASL _position),_checkVisibleDistance,_playersOnGround,[EAST,CIVILIAN,SIDEFRIENDLY],0,0] call (missionNamespace getVariable 'QS_fnc_isPosVisible')) <= 0.25) then {
							_positionFound = TRUE;
						};
					};
				};
		...
		if (_movePos isEqualTo []) then {
			_movePos = [1,(position (leader _grp)),EAST] call (missionNamespace getVariable 'QS_fnc_scGetNearestSector');
		};
		if (!(_movePos isEqualTo [])) then {
				...
				if ((_players inAreaArray [_randomPos,250,250,0,FALSE]) isEqualTo []) then {	
					if ((((_randomPos select [0,2]) nearRoads 25) select {((_x isEqualType objNull) && (!((roadsConnectedTo _x) isEqualTo [])))}) isEqualTo []) then {
						if (!([_randomPos,_centerPos,25] call (missionNamespace getVariable 'QS_fnc_waterIntersect'))) then {
							if (([(AGLToASL _randomPos),_checkVisibleDistance,_playersOnGround,[EAST,CIVILIAN,SIDEFRIENDLY],0,0] call (missionNamespace getVariable 'QS_fnc_isPosVisible')) <= 0.1) then {
								_positionFound = TRUE;
							};
						};
					};
	...
	if (_sectorData isEqualTo -3) then {
		...
		_assaultGrp = createGroup [WEST,TRUE];
