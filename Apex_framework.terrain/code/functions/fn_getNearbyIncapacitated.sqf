...
private _incapacitated = (_origin nearEntities ['CAManBase',_radius]) select {
	( ((lifeState _x) isEqualTo 'INCAPACITATED') && ( (isNull (group _x)) || {(!((side (group _x)) in ((QS_player getVariable ['QS_unit_side',EAST]) call _fn_enemySides)))}) )
};
private _v = objNull;
{
	_v = _x;
		...
		if (((crew _v) findIf {(alive _x)}) isNotEqualTo -1) then {
			{
				if ((lifeState _x) isEqualTo 'INCAPACITATED') then {
					if (isNull (group _x)) then {
						...
						if (!((side (group _x)) in ((QS_player getVariable ['QS_unit_side',EAST]) call _fn_enemySides))) then {
							_incapacitated pushBackUnique _x;
						};
					};
				};
