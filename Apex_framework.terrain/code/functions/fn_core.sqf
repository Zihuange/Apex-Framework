			...
			if ((missionNamespace getVariable 'QS_activeRegion') isNotEqualTo _module_fob_activeRegion) then {
				...
				_supportMessagePopped = _false;
				{
					...
					if (
						...
						0 = ['sideChat',[_east,'HQ'],(format ['%3 %1 %4 %2',_module_fob_respawn_ticketsAdded,(QS_module_fob_flag getVariable ['QS_deploy_tickets',0]),localize 'STR_QS_Chat_041',localize 'STR_QS_Chat_042'])] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];
						if (!isNil {_ve getVariable 'QS_transporter'}) then {
							...
							if (!(_supportMessagePopped)) then {
								...
								['sideChat',[_east,'OPF'],(format [localize 'STR_QS_Chat_043',((_ve getVariable 'QS_transporter') # 0),_dn1])] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];
							};
						};
					};
					// VEHICLE RESPAWN
							...
							if (
								...
								if ((_v distance2D _vpos) > 10) then {
									...
									if (
										...
										{(([_posCheck,25,[_east,_civilian],_allPlayers,0] call _fn_serverDetector) isEqualTo [])}
									) then {
										if (
											(_isDynamicVehicle) || 
											{(_v isKindOf 'Helicopter')}
