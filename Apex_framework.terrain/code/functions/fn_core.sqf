...
_sideMissionList = [
	...
	[
		0.625,							// Side mission frequency weighting  ( https://community.bistudio.com/wiki/selectRandomWeighted )
		0.625,
		0.625,
		0.625,
		0.625,
...
_enemyCAS = TRUE;
_timeLastCAS = 0;
_timeCASdelayFixed = 120;
_timeCASdelayRandom = 240;
...
_QS_module_enemyCAS_spawnDelayDefault = 300;
		...
		if (!(_mainMissionActive)) then {
			...
			if (_timeNow > _aoGraceTime) then {
				if ((missionNamespace getVariable 'QS_mission_aoType') isEqualTo 'CLASSIC') then {
					...
					if (
						([0] call (missionNamespace getVariable 'QS_fnc_aoSubObjectives')) || 
						...
						if (!alive (missionNamespace getVariable 'QS_westCommander')) then {
							if ((markerColor 'QS_marker_hqMarker') isEqualTo 'ColorEAST') then {
								if ((((units _east) inAreaArray [(missionNamespace getVariable 'QS_HQpos'),100,100,0,_false,-1])) isEqualTo []) then {
									if ((count (((units _west) + (units _resistance)) inAreaArray [(missionNamespace getVariable 'QS_HQpos'),50,50,0,_false,-1])) > 1) then {
					...
					if ((missionNamespace getVariable 'QS_mission_aoType') isEqualTo 'SC') then {
						...
						if ((missionNamespace getVariable 'QS_virtualSectors_aoMortars') isNotEqualTo []) then {
							if (((missionNamespace getVariable 'QS_virtualSectors_aoMortars') select {((alive _x) && (alive (gunner _x)))}) isEqualTo []) then {
								['sideChat',[_east,'HQ'],localize 'STR_QS_Chat_040'] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];
									...
									if ((isNull (attachedTo _x)) && (isNull (ropeAttachedTo _x))) then {
										if (!(_module_fob_logistics_respawnEnabled)) then {
											...
											if (_x isKindOf 'Reammobox_F') then {
												...
												_module_fob_respawn_ticketsAdded = 10 + (_x getVariable ['QS_medicalVehicle_reviveTickets',(getNumber (configFile >> 'CfgVehicles' >> (typeOf _x) >> 'transportSoldier'))]);
											};
											...
											0 = ['sideChat',[_east,'HQ'],(format ['%3 %1 %4 %2',_module_fob_respawn_ticketsAdded,(missionNamespace getVariable 'QS_module_fob_respawnTickets'),localize 'STR_QS_Chat_041',localize 'STR_QS_Chat_042'])] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];
											if (!isNil {_x getVariable 'QS_transporter'}) then {
												...
												if (!(_supportMessagePopped)) then {
													...
													['sideChat',[_west,'OPF'],(format [localize 'STR_QS_Chat_043',((_x getVariable 'QS_transporter') # 0),(getText (configFile >> 'CfgVehicles' >> (typeOf _x) >> 'displayName'))])] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];
												};
											};
										};
									};
								...
								if (isNil {_x getVariable 'QS_vehicle_isSuppliedFOB'}) then {
									if ((isNull (attachedTo _x)) && (isNull (ropeAttachedTo _x))) then {
										...
										if (!isNil {_x getVariable 'QS_transporter'}) then {
											...
											if (!(_supportMessagePopped)) then {
												_supportMessagePopped = _true;
												['sideChat',[_east,'OPF'],(format [localize 'STR_QS_Chat_043',((_x getVariable 'QS_transporter') # 0),(getText (configFile >> 'CfgVehicles' >> (typeOf _x) >> 'displayName'))])] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];
											};
										};
									};
								};
								...
								if ((isNull (attachedTo _x)) && (isNull (ropeAttachedTo _x))) then {
									if (isNil {_x getVariable 'QS_vehicle_isSuppliedFOB'}) then {
										...
										if (!isNil {_x getVariable 'QS_transporter'}) then {
											...
											if (!(_supportMessagePopped)) then {
												_supportMessagePopped = _true;
												0 = ['sideChat',[_east,'OPF'],(format [localize 'STR_QS_Chat_043',((_x getVariable 'QS_transporter') # 0),(getText (configFile >> 'CfgVehicles' >> (typeOf _x) >> 'displayName'))])] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];
											};
										};
									};
								};
				...
				if (_module_fob_attackEnabled) then {
					if ((_timeNow > _module_fob_assault_checkDelay) || {(missionNamespace getVariable 'QS_fob_cycleAttack')}) then {
						...
						if (([(markerPos 'QS_marker_module_fob'),50,_enemySides,_allUnits,1] call _fn_serverDetector) > 1) then {
							if (([(markerPos 'QS_marker_module_fob'),100,[_east],_allUnits,1] call _fn_serverDetector) < 1) then {
							...
							if (
								...
								if ((_v distance2D _vpos) > 10) then {
									...
									if (
										...
										if (([_posCheck, ([_QS_vRespawnDist_base,_QS_vRespawnDist_field] select ((_v distance2D _vpos) >= 750)) ,[_east,_civilian],(if ((_v isKindOf 'Helicopter') && {(_allPlayersCount > 15)}) then [{(_allPlayers select {(_x getUnitTrait 'QS_trait_pilot')})},{_allPlayers}]),0] call _fn_serverDetector) isEqualTo []) then {
		...
		if (_timeNow > _QS_module_airDefense_checkDelay) then {
			if (_airDefenseAvailable) then {
				...
				if (_timeNow > (_airDefenseArray # 2)) then {
					...
					['sideChat',[_east,'AirBase'],localize 'STR_QS_Chat_044'] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];	
				};
			};
			_QS_module_airDefense_checkDelay = _timeNow + _QS_module_airDefense_delay;
		};
...
if (!(_resumeScript)) then {
	['hint',(parseText 'Uho！如果出现任何bug，请将下列错误代码提供给管理员：<br/><br/>-=D.R=- Frank<br/><br/>感谢你的帮助！')] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];
};
0 spawn _QS_module_restart_script;