...
if (_type isEqualTo 'GRID_MARKERS') exitWith {
	...
	_objectiveCode = {
		...
		if ( (markerText 'QS_marker_grid_capState') isNotEqualTo (format ['   网格占领进度: %1 / %2',_currentCount,_requiredCount])) then {
...
if (_type isEqualTo 'SITE_TUNNEL') exitWith {
	...
	if (_entities isNotEqualTo []) then {
		...
		_objectiveCode = {
			...
			if ((toLowerANSI (markerText 'QS_marker_grid_rspState')) isNotEqualTo (toLowerANSI (format ['   地道摧毁进度: %1 / %2',((_entitiesTotal - _aliveCount) max 0),(missionNamespace getVariable 'QS_grid_AIRspTotal')]))) then {
