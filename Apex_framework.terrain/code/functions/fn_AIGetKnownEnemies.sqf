	...
	if (canSuspend) then {
		...
		for '_x' from 0 to 1 step 0 do {
			if ((diag_fps > 10) && (!(missionNamespace getVariable ['QS_AI_targetsKnowledge_suspend',_false]))) then {
											...
											if ((_positionError <= 50) || {(_knownVehicle isKindOf 'Air')}) then {		// <= 10
												if (_targetPosition isNotEqualTo [0,0,0]) then {
													...
													if (_targetIndexMem isEqualTo -1) then {
														...
														(missionNamespace getVariable 'QS_AI_targetsKnowledge_WEST') pushBack [_knownUnit,_targetPosition,_knownVehicle,(parseNumber (_positionError toFixed 3)),_unit,(parseNumber ((_time - _timeLastSeen) toFixed 3)),(rating _knownUnit),(_east knowsAbout _knownVehicle)];
													} else {
				...
				missionNamespace setVariable ['QS_AI_targetsKnowledge_WEST',(missionNamespace getVariable 'QS_AI_targetsKnowledge_WEST'),TRUE];		//QS_system_AI_owners
	...
	private _vehicles = [];
	{
		...
		if (alive _vehicle) then {
			if (!(_vehicle isKindOf 'Man')) then {
				if (((crew _vehicle) findIf {((alive _x) && ((side (group _x)) in [EAST]))}) isNotEqualTo -1) then {
	...
	if ((missionNamespace getVariable ['QS_AI_targetsKnowledge_WEST',[]]) isNotEqualTo []) then {
		...
		_targetsKnowledge = missionNamespace getVariable ['QS_AI_targetsKnowledge_WEST',[]];
		{
			if ((random 1) > 0.25) then {
				...
				_targetVehicle = _x # 2;
				if (alive _targetVehicle) then {
					if (((crew _targetVehicle) findIf {((alive _x) && ((side (group _x)) in [EAST]))}) isNotEqualTo -1) then {
