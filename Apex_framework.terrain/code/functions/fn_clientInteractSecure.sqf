...
if (!isNil {_cursorTarget getVariable 'QS_object_GT_0'}) exitWith {
	...
	['sideChat',[EAST,'OPF'],(format [localize 'STR_QS_Chat_030',(groupID (group player)),(['Kavala','Georgetown'] select (worldName isEqualTo 'Tanoa')),localize 'STR_QS_Chat_032'])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
...
if (!isNil {_cursorTarget getVariable 'QS_sc_subObj_1'}) exitWith {
	...
	['sideChat',[EAST,'OPF'],(format ['%1 %2',(groupID (group player)),localize 'STR_QS_Chat_033'])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
...
if (!isNil {_cursorTarget getVariable 'QS_sc_subObj_3'}) exitWith {
	player playAction 'PutDown';
	50 cutText [localize 'STR_QS_Text_136','PLAIN DOWN',0.75];
	['sideChat',[EAST,'OPF'],(format ['%1 %2',(groupID (group player)),localize 'STR_QS_Chat_034'])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
...
player playAction 'PutDown';
['sideChat',[EAST,'OPF'],(format ['%1 %2',(groupID (group player)),localize 'STR_QS_Chat_035'])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
