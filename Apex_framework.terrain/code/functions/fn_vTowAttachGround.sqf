...
if ((_attachPoint isEqualTo _defaultAttachPoint) || {((count _attachPointReturn) > 2)}) then {
	...
	50 cutText [format ['正在牵引 %1',_displayName],'PLAIN DOWN',0.5];
	_checkIntersections = TRUE;
} else {
	_isHauling = TRUE;
	_vehicle setVariable ['QS_vehicle_hauling',[TRUE,_towedVehicle],TRUE];
	50 cutText [format ['正在拖运 %1',_displayName],'PLAIN DOWN',0.5];
};
if (_towedVehicle isKindOf 'StaticWeapon') then {
...
if (_isHauling && (_towedVehicle isKindOf 'StaticWeapon') && (!(_towedVehicle isKindOf 'Pod_Heli_Transport_04_crewed_base_F'))) exitWith {
	...
	50 cutText [format ['组装 %1',_displayName],'PLAIN DOWN',0.5];
};
if (_isUAV) then {
	_vehicle setVariable [
		'QS_action_towRelease1',
		(_vehicle addAction ['放开','[cameraOn] call QS_fnc_vTowRelease',[],21,FALSE,TRUE,'','[cameraOn] call QS_fnc_conditionTowDetach']),
		FALSE
	];
	...
	_vehicle setVariable [
		...
		(player addAction ['放开','[(vehicle player)] call QS_fnc_vTowRelease',[],21,FALSE,TRUE,'','[(vehicle player)] call QS_fnc_conditionTowDetach']),
		FALSE
	];
...
if (isNull (attachedTo _towedVehicle)) then {
	if ((_towedVehicle distance2D (markerPos 'QS_marker_crate_area')) < 500) then {
		if (!isNil {_towedVehicle getVariable 'QS_vehicle_isSuppliedFOB'}) then {
			...
			systemChat format ['%1 重置 FOB 补给',(getText (configFile >> 'CfgVehicles' >> (typeOf _towedVehicle) >> 'displayName'))];
		};
	};
};
if (!isNil {_towedVehicle getVariable 'QS_loadCargoIn'}) exitWith {
	...
	if ((_towedVehicle getVariable 'QS_loadCargoIn') setVehicleCargo _towedVehicle) then {
		_text = format ['%1 装载到 %2',(getText (configFile >> 'CfgVehicles' >> (typeOf _towedVehicle) >> 'displayName')),(getText (configFile >> 'CfgVehicles' >> (typeOf (_towedVehicle getVariable 'QS_loadCargoIn')) >> 'displayName'))];
	} else {
		_text = format ['装载失败， %1 进入 %2',(getText (configFile >> 'CfgVehicles' >> (typeOf _towedVehicle) >> 'displayName')),(getText (configFile >> 'CfgVehicles' >> (typeOf (_towedVehicle getVariable 'QS_loadCargoIn')) >> 'displayName'))];
	};
...
50 cutText ['已放开','PLAIN DOWN',0.25];
uiSleep 0.5;
