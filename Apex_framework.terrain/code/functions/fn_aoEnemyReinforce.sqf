...
if (
	...
	{((random 1) > 0.5)} &&
	{
		_validData = ((missionNamespace getVariable 'QS_ao_urbanSpawn_data') select {
			...
			{((count (_unitsWEST inAreaArray [(getPosATL (_x # 0)),25,25,0,FALSE])) <= 12)}
		});
...
if (
	...
	{(([4,WEST,_aoPos,2000] call (missionNamespace getVariable 'QS_fnc_AIGetKnownEnemies')) < 2)} &&
	{(([3,WEST,_aoPos,2000] call (missionNamespace getVariable 'QS_fnc_AIGetKnownEnemies')) < 2)}
) exitWith {
