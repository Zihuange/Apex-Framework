...
_onCompleted = {
	...
	if (player getUnitTrait 'QS_trait_JTAC') then {
		...
		if ((!(_cursorObject in ([(listRemoteTargets (player getVariable ['QS_unit_side',WEST])),0] call (missionNamespace getVariable 'QS_fnc_listRemoteTargets')))) && (!(_cursorObject getVariable ['QS_remoteTarget_reported',FALSE]))) then {
			...
			['sideChat',[WEST,'BLU'],(format ['%1 (JTAC) 标记了一个位于 %3 的 %2 并请求火力打击将其摧毁。',profileName,(getText (configFile >> 'CfgVehicles' >> (typeOf _cursorObject) >> 'displayName')),(mapGridPosition _cursorObject)])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
		...
		if ((!(_cursorObject in ([(listRemoteTargets (player getVariable ['QS_unit_side',WEST])),0] call (missionNamespace getVariable 'QS_fnc_listRemoteTargets')))) && (!(_cursorObject getVariable ['QS_remoteTarget_reported',FALSE]))) then {
			...
			50 cutText ['目标已标记','PLAIN DOWN',0.3];
			playSound 'beep_target';
			['sideChat',[WEST,'BLU'],(format ['%1 (JTAC) 标记了一个位于 %3 的 %2 并请求火力打击将其摧毁。',profileName,(groupID (group player)),(getText (configFile >> 'CfgVehicles' >> (typeOf _cursorObject) >> 'displayName')),(mapGridPosition _cursorObject)])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
			[77,'FIRE_SUPPORT',[_cursorObject,profileName],FALSE] remoteExec ['QS_fnc_remoteExec',2];
		} else {
			50 cutText ['目标已被标记过','PLAIN DOWN',0.3];
		};	
