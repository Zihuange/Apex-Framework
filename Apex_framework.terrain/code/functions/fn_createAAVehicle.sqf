			...
			if (!isNull _instigator) then {
				if (isPlayer _instigator) then {
					if ((vehicle _instigator) isKindOf 'Air') then {
						...
						['sideChat',[EAST,'OPF'],(format ['%1 %4 %2 %5 %3!',_name,_objDisplayName,_killerDisplayName,localize 'STR_QS_Chat_045',localize 'STR_QS_Chat_046'])] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
					};
				};
			};
...
if (alive _vehicle) then {
	...
	if ((random 1) > 0.5) then {
		_grp = [(_position getPos [(random 30),(random 360)]),(random 360),WEST,'OI_support_ENG2',FALSE] call (missionNamespace getVariable 'QS_fnc_spawnGroup');
		{
			//if ((random 1) < 0.666) then {
