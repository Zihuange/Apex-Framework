	...
	if (!isNull _cameraOn) then {
		...
		if (freeLook) then {
			...
			} forEach (_player nearEntities [['CAManBase','LandVehicle','Air','Ship','StaticWeapon'],50]);
			{
				...
				if (
					(!isNull (group _unit)) &&
					{((side (group _unit)) isEqualTo (_player getVariable ['QS_unit_side',EAST]))} &&
			...
			if (
				...
				{(
					(
						...
						{(
							((_cursorTarget isKindOf 'CAManBase') && {((side (group _cursorTarget)) isEqualTo (_player getVariable ['QS_unit_side',EAST]))}) ||
							(!(_cursorTarget isKindOf 'CAManBase') && {((side (assignedGroup _cursorTarget)) isEqualTo (_player getVariable ['QS_unit_side',EAST]))})
						)} &&
					...
					{(
						...
						{(
							...
							{((!isNull (assignedGroup _cursorTarget)) && {((side (assignedGroup _cursorTarget)) isEqualTo (_player getVariable ['QS_unit_side',EAST]))})} || 
