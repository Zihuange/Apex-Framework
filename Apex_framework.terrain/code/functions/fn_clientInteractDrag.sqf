...
if (_t getVariable ['QS_interaction_disabled',FALSE]) exitWith {
	50 cutText ['此物品无法交互','PLAIN',0.3];
};
if (_t getVariable ['QS_unit_needsStabilise',FALSE]) exitWith {
	50 cutText ['此单位伤势过重，需要先稳定伤势','PLAIN',0.3];
};
...
if (_t isKindOf 'CAManBase') exitWith {
	...
	50 cutText [(format ['正在拖拽 %1',(name _t)]),'PLAIN DOWN',0.3];
...
player playActionNow 'grabDrag';
_text = format ['正在拖拽 %1',(_t getVariable ['QS_ST_customDN',(getText (configFile >> 'CfgVehicles' >> (typeOf _t) >> 'displayName'))])];
...
if ((uiNamespace getVariable ['QS_dragStuckMsg',-1]) isEqualTo -1) then {
	...
	(missionNamespace getVariable 'QS_managed_hints') pushBack [5,TRUE,10,-1,'如果无法拖动，尝试双击[C]',[],-1,TRUE,'Help',TRUE];
};
TRUE;