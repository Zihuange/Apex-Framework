	...
	if (isNull (objectParent player)) then {
		if (!(unitIsUav cameraOn)) then {
			if ((cameraOn distance (markerPos 'QS_marker_base_marker')) < 300) then {
				50 cutText [(localize 'STR_QS_KeyDown_throw'),'PLAIN DOWN'];
				_c = TRUE;
			};
		};
	};
...
if (_key in (actionKeys 'PushToTalk')) then {
	if (currentChannel isNotEqualTo 5) then {
		if ('ItemRadio' in (assignedItems player)) then {
			if (currentChannel in [0,1,6]) then {
				if (!((getPlayerUID player) in (['ALL'] call (missionNamespace getVariable 'QS_fnc_whitelist')))) then {
					if (!(player getUnitTrait 'QS_trait_HQ')) then {
						setCurrentChannel 5;
						50 cutText [(localize 'STR_QS_KeyDown_pushToTalk')];
						_c = TRUE;
					};
				};
			} else {
						...
						if (!(player getUnitTrait 'QS_trait_pilot')) then {
							if (!(player getUnitTrait 'uavhacker')) then {
								if (!(player getUnitTrait 'QS_trait_HQ')) then {
									if (!((getPlayerUID player) in (['ALL'] call (missionNamespace getVariable 'QS_fnc_whitelist')))) then {
										setCurrentChannel 5;
										50 cutText [(localize 'STR_QS_KeyDown_pushToTalkPilot'),'PLAIN DOWN'];
										_c = TRUE;
									};
								};
							};
			...
			setCurrentChannel 5;
			50 cutText [(localize 'STR_QS_KeyDown_pushToTalkNoRadio'),'PLAIN DOWN'];
			_c = TRUE;
		};
	};
};
			...
			if (!(cameraView in ['INTERNAL','GUNNER'])) then {
				if ((lifeState player) isNotEqualTo 'INCAPACITATED') then {
					if ((missionNamespace getVariable ['QS_missionConfig_aoType','CLASSIC']) in ['CLASSIC','SC','GRID']) then {
						50 cutText [(localize 'STR_QS_KeyDown_personView'),'PLAIN DOWN',0.5];
			...
			if (((count (crew _v))) > 1) then {
				if (diag_tickTime > (player getVariable ['QS_client_lastAutoHoverMsg',-1])) then {
					...
					if (_arrayToSend isNotEqualTo []) then {
						[63,[5,[(format [(localize 'STR_QS_KeyDown_autoHover'),profileName]),'PLAIN DOWN',0.3]]] remoteExec ['QS_fnc_remoteExec',_arrayToSend,FALSE];
					};
				};
			};
		};
					...
					if ((lifeState player) in ['HEALTHY','INJURED']) then {
						if (time > (player getVariable ['QS_client_lastGesture',time])) then {
							...
							if (_key isEqualTo 0x4C) exitWith {
								...
								50 cutText [(localize 'STR_QS_KeyDown_No'),'PLAIN DOWN',0.1];
							};
							if (_key isEqualTo 0x4B) exitWith {
								player playActionNow (selectRandom ['gestureGo']);
								50 cutText [(localize 'STR_QS_KeyDown_Go'),'PLAIN DOWN',0.1];
								_order = (localize 'STR_QS_KeyDown_Go') + (localize 'STR_QS_KeyDown_groupOrder');
							};
							if (_key isEqualTo 0x47) exitWith {
								player playActionNow (selectRandom ['gesturePoint','gestureAdvance']);
								50 cutText [(localize 'STR_QS_KeyDown_Advance'),'PLAIN DOWN',0.1];
								_order = (localize 'STR_QS_KeyDown_Advance') + (localize 'STR_QS_KeyDown_groupOrder');
							};
							if (_key isEqualTo 0x48) exitWith {
								player playActionNow (selectRandom ['gestureNod']);
								50 cutText [(localize 'STR_QS_KeyDown_Nod'),'PLAIN DOWN',0.1];
							};
							if (_key isEqualTo 0x49) exitWith {
								player playActionNow (selectRandom ['gestureFreeze']);
								50 cutText [(localize 'STR_QS_KeyDown_Freeze'),'PLAIN DOWN',0.1];
								_order = (localize 'STR_QS_KeyDown_Hold') + (localize 'STR_QS_KeyDown_groupOrder');
							};
							if (_key isEqualTo 0x4D) exitWith {
								player playActionNow (selectRandom ['gestureHi']);
								50 cutText [(localize 'STR_QS_KeyDown_Hi'),'PLAIN DOWN',0.1];
							};
							if (_key isEqualTo 0x51) exitWith {
								player playActionNow (selectRandom ['gestureCeaseFire']);
								50 cutText [(localize 'STR_QS_KeyDown_Ceasefire'),'PLAIN DOWN',0.1];
								_order = (localize 'STR_QS_KeyDown_Ceasefire') + (localize 'STR_QS_KeyDown_groupOrder');
							};
							if (isNull (objectParent player)) then {
								if ((count (units (group player))) > 1) then {
									if (player isEqualTo (leader (group player))) then {
										if (_order isNotEqualTo '') then {
											...
											if (_arrayToSend isNotEqualTo []) then {
												[63,[5,[(format [(localize 'STR_QS_KeyDown_leader'),profileName,_order]),'PLAIN DOWN',0.333]]] remoteExec ['QS_fnc_remoteExec',_arrayToSend,FALSE];
											};
										};
									};
								};
		...
		if (player isEqualTo (effectiveCommander (vehicle player))) then {
			if (isNil {player getVariable 'QS_pilot_rappellSafety'}) then {
				...
				if (isNil {(vehicle player) getVariable 'QS_rappellSafety'}) then {
					...
					50 cutText [(localize 'STR_QS_KeyDown_fastropeDisabled'),'PLAIN DOWN',1];
				} else {
					(vehicle player) setVariable ['QS_rappellSafety',nil,TRUE];
					50 cutText [(localize 'STR_QS_KeyDown_fastropeEnabled'),'PLAIN DOWN',1];
				};
				0 spawn {
					uiSleep 4.5;
