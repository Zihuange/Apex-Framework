	...
	for '_x' from 0 to 1 step 0 do {
		...
		if (
			(
				...
				{(([(AGLToASL _position1),_checkVisibleDistance,_playersOnGround,[EAST,CIVILIAN,SIDEFRIENDLY],0,0] call (missionNamespace getVariable 'QS_fnc_isPosVisible')) <= 0.1)}
			) || 
			{(_iterations > 300)}
		) exitWith {};
